...
void GetBlock::Start()
{
  ...
  suckmotor[0].ConfigPID().AsPosC().Pos_Coeff(4.0f, 0.0f, 0.0f) // 位置环 kp/ki/kd（待整定）
      .Pos_Limit(300.0f, 200.0f)                                // 位置环积分限幅、输出速度限幅（rad/s）
      .Spd_Coeff(0.15f, 0.005f, 0.0f)                           // 速度环 kp/ki/kd（待整定）
      .Spd_Limit(2.0f, 10.0f)                                   // 速度环积分限幅、电流输出限幅（code）
      .CurLimit(10)
      .Apply();
  suckmotor[0].driver.Enable();

  // ---- 大疆吸吮电机右（M2006，减速比36，CAN2 ID:2，位置串级模式）----
  suckmotor[1].Init(Hardware::hcan_sub, 2, DJI_C610);
  suckmotor[1].ConfigPID().AsPosC().Pos_Coeff(4.0f, 0.0f, 0.0f) // 位置环 kp/ki/kd（待整定）
      .Pos_Limit(300.0f, 200.0f)                                // 位置环积分限幅、输出速度限幅（rad/s）
      .Spd_Coeff(0.15f, 0.005f, 0.0f)                           // 速度环 kp/ki/kd（待整定）
      .Spd_Limit(2.0f, 10.0f)                                   // 速度环积分限幅、电流输出限幅（code）
      .CurLimit(10)
      .Apply();
  suckmotor[1].driver.Enable();

  // ---- 大疆伸缩电机左（M2006，减速比36，CAN2 ID:4，位置串级模式）----
  stretchmotor[0].Init(Hardware::hcan_sub, 4, DJI_C610);
  stretchmotor[0].ConfigPID().AsPosC().Pos_Coeff(4.0f, 0.0f, 0.0f) // 位置环 kp/ki/kd（待整定）
      .Pos_Limit(300.0f, 200.0f)                                   // 位置环积分限幅、输出速度限幅（rad/s）
      .Spd_Coeff(0.15f, 0.005f, 0.0f)                              // 速度环 kp/ki/kd（待整定）
      .Spd_Limit(2.0f, 10.0f)                                      // 速度环积分限幅、电流输出限幅（code）
      .CurLimit(10)
      .Apply();
  stretchmotor[0].driver.Enable();

  // ---- 大疆伸缩电机右（M2006，减速比36，CAN2 ID:3，位置串级模式）----
  stretchmotor[1].Init(Hardware::hcan_sub, 4, DJI_C610);
  stretchmotor[1].ConfigPID().AsPosC().Pos_Coeff(4.0f, 0.0f, 0.0f) // 位置环 kp/ki/kd（待整定）
      .Pos_Limit(300.0f, 200.0f)                                   // 位置环积分限幅、输出速度限幅（rad/s）
      .Spd_Coeff(0.15f, 0.005f, 0.0f)                              // 速度环 kp/ki/kd（待整定）
      .Spd_Limit(2.0f, 10.0f)                                      // 速度环积分限幅、电流输出限幅（code）
      .CurLimit(10)
      .Apply();
  stretchmotor[1].driver.Enable();
